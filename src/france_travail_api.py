...
class FranceTravailClient:
    ...
    def __init__(self, client_id: Optional[str] = None, client_secret: Optional[str] = None, logger: logging.Logger = logging.getLogger()):
        ...
        :param logger: L'instance de logger à utiliser.
        """
    ...
    async def _get_access_token(self) -> bool:
        """
        Obtient un nouveau token d'accès auprès du serveur d'authentification.
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(AUTH_URL, headers=headers, data=data, timeout=10) as response:
                    ...
                    # Garder une marge de 60 secondes avant l'expiration réelle
    ...
    async def search_offers_async(self, search_term: str, max_offers: int = 150) -> List[Dict]:
        """
        Recherche des offres d'emploi de manière asynchrone.
        ...
        if not self._is_token_valid():
            if not await self._get_access_token():
                return [] # Impossible de procéder sans token
        
        headers = {'Authorization': f'Bearer {self._access_token}'}
        # sort=1 trie par pertinence
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(url, headers=headers, params=params, timeout=20) as response:
                    ...
                    # Formatter les offres dans un format simple et unifié
                    return [{
            ...
            self.logger.error(f"France Travail : Erreur lors de la recherche async. Erreur: {e}")
            return []